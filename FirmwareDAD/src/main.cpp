...
void POST_tests()
{
  ...
  String sensor_value_body = serializeSensorValueBody(temperatura,1369,millis());
  ...
  String actuator_states_body = serializeActuatorStatusBody(actuador,actuador,2,millis());
...
void ConnectMqtt()
{
  ...
  {
    client.subscribe("mqttChannelDevice3");                                    
...
if(repeticiones == 1000){
  ...
  if (event.temperature > TEMPERATURE_THRESHOLD || valueSensor > TEMPERATURE_THRESHOLD) {
    // Activar el led
